...
function OrderDetailsTable({order, paypalClientId, isAdmin, stripeClientSecret}: {order: Omit<Order, "paymentResult">, paypalClientId: string, isAdmin: boolean, stripeClientSecret:string | null}) {
            ...
            <div className="col-span-2 space-y-4 overflow-x-auto">
                <Card>
                    <CardContent className="p-4 gap-4">
                        <h2 className="text-xl pb-4">Paymet Method</h2>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">ItemPrice </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {orderItem.map((item) => (
            ...
            <Card>
                <CardContent className="p-4 gap-4 space-y-4">
                    <div className="flex justify-between">
                        <div>Items</div>
                        <div>{formatterCurrency(itemsPrice)}</div>
                    </div>
                    <div className="flex justify-between">
                        <div>Tax</div>
                        <div>{formatterCurrency(taxPrice)}</div>
                    </div>
                    <div className="flex justify-between">
                        <div>Shipping</div>
                        <div>{formatterCurrency(shippingPrice)}</div>
                    </div>
                    <div className="flex justify-between">
                        <div>Total</div>
                        <div>{formatterCurrency(totalPrice)}</div>
                    </div>
                    {/* Paypal paymet */}
